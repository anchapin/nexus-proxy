...
`.github/workflows/ci.yml` runs two benchmark jobs on every PR:

1. **`bench (smoke)`** — `make bench-short` with `continue-on-error: true`.
   Uploads `bench.txt` as a build artifact for manual inspection.

2. **`bench (regression check)`** — runs benchmarks, compares against
   the stored baseline (`bench/baseline.txt`) with `benchstat`, and posts
   the comparison table as a PR comment via
   `marocchino/sticky-pull-request-comment`. Also non-blocking
   (`continue-on-error: true`). Flags regressions exceeding
   `NEXUS_BENCH_REGRESSION_THRESHOLD` (default 15%) on `ns/op`.

Both jobs are non-blocking while the baseline stabilises. Once the team
has confidence in the CI-runner numbers, the `bench-regression` job can
be promoted to gating.
...
## 10. Benchstat regression detection (issue #1186)

### Stored baseline

`bench/baseline.txt` is a committed benchmark snapshot generated by:

```bash
make bench-baseline
# Which runs:
# go test -run='^$' -bench=. -benchmem -count=10 -benchtime=100ms ./...
```

The file uses the standard `go test -bench` output format that
`benchstat` parses directly. It includes `-count=10` for tighter
statistical confidence and `-benchtime=100ms` to keep total runtime
under ~90s.

### When to regenerate

Regenerate the baseline **on `develop` merges**, not on every PR:
...
1. Runs `go test -run='^$' -bench=. -benchmem -count=5 -benchtime=100ms ./...`
2. Installs `benchstat` (`golang.org/x/perf/cmd/benchstat` — pure Go, CGO-free)
3. Runs `benchstat bench/baseline.txt pr.txt`
4. Posts the comparison table as a PR comment (updated on re-runs)
5. Flags benchmarks where `ns/op` regressed by more than
   `NEXUS_BENCH_REGRESSION_THRESHOLD` (default **15%**)
...
```
                                    │ pr.txt              │
                                    │ sec/op              │
Record_SmallEntries-12                147.90n ± 3%
Record_LargeWindow-12                2.960µ ± 2%
SerializeToTOON/3rows-12             11.70µ ± 5%  +12.3%  p=0.001
```

* **No delta column** — the benchmark is within noise; no change detected.
* **`+N.N%`** — the PR's result is slower by that percentage.
* **`-N.N%`** — the PR's result is faster by that percentage.
* **`p=0.0NN`** — statistical significance; `p < 0.05` means the change
  is real, not noise.
* **`~`** — the variance is too high to draw a conclusion.

### Threshold rationale

The default 15% threshold balances sensitivity against CI-runner noise.
GitHub Actions `ubuntu-latest` runners have variable performance (±10%
is common), so a threshold below ~10% would produce false positives.
Raise the threshold if the team finds the noise floor is higher; lower
it once the baseline is regenerated from a CI run.

### Known limitations

* **Hardware mismatch.** The initial baseline was generated on an AMD
  Ryzen 5 5600G, not a CI runner. PR-to-CI comparisons will show
  systematic offsets until the baseline is regenerated from a CI run.
  This is expected; the deltas are still meaningful for relative
  comparisons within the same CI job family.
* **`BenchmarkChatRouteFrontierStream`** requires a live frontier
  endpoint and is excluded from the baseline. It returns 502 without an
  API key configured.
* **CGO-free.** `benchstat` is pure Go; no new CGO dependency is
  introduced.
