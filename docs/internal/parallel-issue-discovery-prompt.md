...
that advance `nexus-proxy` toward its goals.
...
- **Latest merged PR**: #784 (judge stats), #783 (routing-preview), #782 (arbiter cache), #781 (batch embedding), #780 (TOON), #779 (GPU selection), #777 (latency percentiles), #778 (API-key rate limiting)
- **Latest issue number**: ~785 (confirm with `gh issue list --state all --limit 5 --sort created`)
