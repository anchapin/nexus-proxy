...
// changingDimEmbedder returns embeddings whose dimension changes on the second call.
// It simulates an embedder model version change. Each call generates a unique
// embedding by incorporating the text hash so different prompts produce different
// vectors even at the same dimension.
...
func (c *changingDimEmbedder) EmbedBatch(_ context.Context, texts []string) ([][]float64, error) {
	result := make([][]float64, len(texts))
	for i := range texts {
		emb, _ := c.Embed(context.Background(), texts[i])
		result[i] = emb
	}
	return result, nil
}
...
func TestSLMCache_DimMismatch_getSemantic_SkipsMismatchedEntry(t *testing.T) {
	...
	// Set second prompt — stores 2-dim embedding (embedder switched dims).
	...
	if ok || kind != "" {
		t.Errorf("dim mismatch should produce miss: got (%v, %v, %v), want (\"\", false, \"\")", got, ok, kind)
	}
...
func TestSLMCache_DimMismatch_MultipleMismatches(t *testing.T) {
	...
	// Use a dim-changing embedder where calls 2+ return 2-dim.
	emb := &changingDimEmbedder{
		dimA: 4,
		dimB: 2,
		vecA: []float64{1.0, 0.0, 0.0, 0.0},
	...
	// Store two entries with 4-dim embeddings via SetEmbedding (no embedder call).
	c.SetEmbedding("entry-a", RouteLocal, []float64{1.0, 0.0, 0.0, 0.0})
	c.SetEmbedding("entry-b", RouteFrontier, []float64{0.0, 1.0, 0.0, 0.0})

	// Make one embedder call via Set to advance calls to 1 (4-dim), then
	// reset so subsequent Get triggers calls >= 2 which return 2-dim.
	// Actually: SetEmbedding does NOT call the embedder. So calls is still 0.
	// First Get call will be calls=1 -> returns 4-dim. That defeats the test.
	//
	// Instead: use Set (not SetEmbedding) for the first entry to advance calls.
	// After Set("entry-a"), calls=1. Then reset calls to 0, so Get returns 2-dim.
	// But wait, Set calls the embedder which would store 4-dim for entry-a,
	// and then we'd have 4-dim stored and 2-dim query. That's correct.
	//
	// Actually let me just use a 2-dim stub embedder that always returns 2-dim
	// for the query. The stored entries are 4-dim via SetEmbedding. Query is 2-dim.

	// Store entries with 4-dim via SetEmbedding (embedder NOT called).
	// calls is still 0.

	// Now trigger getSemantic with a 2-dim query by calling Set once to advance
	// calls to 1 (returns 4-dim), then SetEmbedding again to not call embedder,
	// then reset calls and Get which returns 2-dim.
	//
	// Simpler: just use a stubEmbedder that always returns 2-dim for the query
	// but we stored 4-dim via SetEmbedding. The query will be 2-dim from stub.

	// OK new approach: use changingDimEmbedder where dimA=4, dimB=2.
	// Set entry-a and entry-b via Set (calls become 1,2 -> 4-dim, 2-dim).
	// calls is now 2. Reset to 0. Get triggers Embed -> calls=1 -> 4-dim.
	// Still not right...
	//
	// Final approach: SetEmbedding stores 4-dim. calls stays 0. Get("query") calls
	// Embed once: calls=1 -> dimA=4. So query is 4-dim, stored is 4-dim -> no mismatch.
	//
	// To get 2-dim query: need calls >= 2 when Get is called. So:
	// SetEmbedding for entry-a (calls still 0)
	// SetEmbedding for entry-b (calls still 0)
	// Set one more time for a different key -> calls becomes 1 -> 4-dim
	// Reset calls = 0
	// Get -> calls=1 -> 4-dim again!
	//
	// Hmm. Let me try a different approach. Use SetEmbedding with nil to advance
	// calls, then reset:
	// SetEmbedding stores nil emb but DOES NOT call embedder.
	// Only Set() and getSemantic's embedder call actually call Embed.
	//
	// OK I'll just write a dedicated embedder for this test.

	// Use a stub that always returns 2-dim. We store 4-dim entries directly.
	stub := &vectorEmbedder{vec: []float64{1.0, 0.0}} // 2-dim
	c2 := NewSLMCacheWithEmbedder(time.Hour, 0, stub, 0.5)

	c2.SetEmbedding("entry-a", RouteLocal, []float64{1.0, 0.0, 0.0, 0.0})
	c2.SetEmbedding("entry-b", RouteFrontier, []float64{0.0, 1.0, 0.0, 0.0})

	// Trigger getSemantic — stub returns 2-dim, stored entries are 4-dim.
	c2.Get(ctx, "query")

	if stats := c2.Stats(); stats.DimMismatch != 2 {
		t.Errorf("DimMismatch = %d after two mismatches, want 2", stats.DimMismatch)
	}
}

func TestSLMCache_DimMismatch_MatchingDimsNoIncrement(t *testing.T) {
	// When dimensions match, DimMismatch must not be incremented.
	stub := newStubEmbedder()
	c := NewSLMCacheWithEmbedder(time.Hour, 0, stub, 0.5)
	ctx := context.Background()

	stub.embeddings["cached"] = []float64{1.0, 0.0, 0.0, 0.0}
	stub.embeddings["query"] = []float64{0.9, 0.1, 0.0, 0.0}

	c.Set(ctx, "cached", RouteLocal)

	// Reset calls to re-embed query with same stub (same dimensions).
	stub.calls = nil

	got, ok, kind := c.Get(ctx, "query")
	if !ok || got != RouteLocal || kind != CacheHitSemantic {
		t.Errorf("semantic match failed: got (%v, %v, %v), want (RouteLocal, true, CacheHitSemantic)", got, ok, kind)
	}

	if stats := c.Stats(); stats.DimMismatch != 0 {
		t.Errorf("DimMismatch = %d with matching dims, want 0", stats.DimMismatch)
	}
}

func TestSLMCache_DimMismatch_SetEmbeddingMismatch(t *testing.T) {
	// SetEmbedding with a different dimension than stored entries must
	// produce dimension mismatches when getSemantic is triggered.
	stub := newStubEmbedder()
	c := NewSLMCacheWithEmbedder(time.Hour, 0, stub, 0.5)
	ctx := context.Background()

	// Store an entry with 4-dim embedding via Set (uses stub, dim=4).
	stub.embeddings["stored"] = []float64{1.0, 0.0, 0.0, 0.0}
	c.Set(ctx, "stored", RouteLocal)

	// Now inject a 2-dim embedding via SetEmbedding directly (simulates
	// the embedder model having changed since the first entry was stored).
	c.SetEmbedding("new-entry", RouteFrontier, []float64{1.0, 0.0})

	// Reset stub so its next Embed returns 2-dim (simulating model change).
	stub.calls = nil
	delete(stub.embeddings, "query")

	// Get on "new-entry" is an exact match — no dimension issue.
	got, ok, kind := c.Get(ctx, "new-entry")
	if !ok || got != RouteFrontier || kind != CacheHitExact {
		t.Errorf("exact match failed: got (%v, %v, %v), want (RouteFrontier, true, CacheHitExact)", got, ok, kind)
	}

	// Get on "query" triggers getSemantic — 2-dim query vs 4-dim "stored" → mismatch.
	_, ok, kind = c.Get(ctx, "query")
	if ok || kind != "" {
		t.Errorf("dim mismatch should produce miss: got (%v, %v, %v), want (\"\", false, \"\")", got, ok, kind)
	}
...
func TestSLMCache_DimMismatch_ZeroDimsIgnored(t *testing.T) {
	// Nil embeddings are skipped at the nil-check before the dimension
	// check runs, so they do not increment DimMismatch (issue #968).
	stub := newStubEmbedder()
	c := NewSLMCacheWithEmbedder(time.Hour, 0, stub, 0.5)
	ctx := context.Background()
	...
	// Store an entry with nil embedding — nil emb is skipped (not compared).
	c.SetEmbedding("nil-emb", RouteLocal, nil)

	// Now Set a fresh entry that uses the stub's fallback embedding (4-dim).
	c.Set(ctx, "fresh-entry", RouteFrontier)

	// The "cached" entry has 4-dim, which matches the fresh query embedding.
	// The "nil-emb" entry is skipped at the nil check. No dimension mismatch
	// should be recorded.
	if stats := c.Stats(); stats.DimMismatch != 0 {
		t.Errorf("DimMismatch = %d with matching dims and nil entry, want 0", stats.DimMismatch)
	}
}
