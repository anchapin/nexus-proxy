...
func TestCategorize(t *testing.T) {
	cases := []struct {
		...
		// DevOps category (issue #1405)
		{"devops_docker", "write a dockerfile for the node app", CategoryDevOps},
		{"devops_kubernetes", "deploy this to kubernetes", CategoryDevOps},
		{"devops_terraform", "set up the infrastructure with terraform", CategoryDevOps},
		{"devops_helm", "configure helm chart for the service", CategoryDevOps},
		{"devops_ingress", "set up ingress controller routing", CategoryDevOps},
		{"devops_deploy", "rollback the deployment to the previous version", CategoryDevOps},
		{"devops_canary", "configure canary deployment strategy", CategoryDevOps},
		// Conversation category (issue #1405)
		{"conversation_continue", "continue with the implementation", CategoryConversation},
		{"conversation_follow_up", "follow up on the previous question", CategoryConversation},
		{"conversation_keep_going", "keep going with the same topic", CategoryConversation},
		{"conversation_extending", "extending the previous work", CategoryConversation},
