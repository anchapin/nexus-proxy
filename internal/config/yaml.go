...
// lower-case RFC-603-style aliases for the corresponding NEXUS_ env var
...
func LoadYAML(path string) (Config, error) {
	...
	// The logic is identical to Load() but reads from os.Getenv
	// directly (not getEnv) so that an explicitly empty env var
	// overrides a non-empty YAML value — matching the "env always wins"
	// contract. We only skip the override when the env var is unset.
...
func (yc YAMLConfig) toConfig() Config {
	cfg := Config{
		...
		TelemetryPath:   yc.stringDefaultAllowEmpty(yc.TelemetryPath, "./nexus-telemetry.jsonl"),
		MetricsDBPath:   yc.stringDefault(yc.MetricsDBPath, DefaultMetricsDBPath()),
	}

	// RAG
	if yc.RAGThreshold > 0 {
		cfg.RAGThreshold = yc.RAGThreshold
	} else {
		cfg.RAGThreshold = 0.55
	}
	cfg.RAGDBPath = yc.stringDefault(yc.RAGDBPath, DefaultRAGDBPath())
	cfg.RAGEmbedCacheSize = yc.intDefault(yc.RAGEmbedCacheSize, 256)

	// Embedder
	cfg.EmbedderType = ragpkg.EmbedderType(strings.ToLower(strings.TrimSpace(
		yc.stringDefault(string(yc.EmbedderType), "ollama"))))
	...
	// Routing
	cfg.TokenGuardrail = yc.intDefault(yc.TokenGuardrail, 6000)
	cfg.SLMTimeout = yc.durationDefault(yc.SLMTimeout, 8*time.Second)
	cfg.SLMCacheMaxEntries = yc.intDefault(yc.SLMCacheMaxEntries, 512)
	cfg.SLMCacheTTL = yc.durationDefault(yc.SLMCacheTTL, 30*time.Second)
	cfg.SLMCacheSemanticThreshold = clampFloat(yc.floatDefault(yc.SLMCacheSemanticThreshold, 0.0), 0, 1)
	cfg.FusionTimeout = yc.durationDefault(yc.FusionTimeout, 120*time.Second)
	cfg.CascadeTimeout = yc.durationDefault(yc.CascadeTimeout, 30*time.Second)
	cfg.ArbiterTimeout = yc.durationDefault(yc.ArbiterTimeout, 60*time.Second)

	// Fusion
	cfg.FusionProgressiveDelivery = yc.boolDefault(yc.FusionProgressiveDelivery, true)
	cfg.FusionAgreementThreshold = yc.floatDefault(yc.FusionAgreementThreshold, 0.85)

	// Judge
	cfg.JudgeURL = yc.stringDefault(yc.JudgeURL, "https://api.z.ai/v1/chat/completions")
	cfg.JudgeModel = yc.stringDefault(yc.JudgeModel, cfg.FrontierModel)
	cfg.JudgeAPIKey = yc.JudgeAPIKey
	cfg.JudgeSampleRate = yc.floatDefault(yc.JudgeSampleRate, 0.1)
	cfg.JudgeConcurrency = yc.intDefault(yc.JudgeConcurrency, 2)
	cfg.JudgeQueueDepth = yc.intDefault(yc.JudgeQueueDepth, 64)
	cfg.JudgeTimeout = yc.durationDefault(yc.JudgeTimeout, 30*time.Second)
	cfg.JudgeCostPer1KUSD = yc.floatDefault(yc.JudgeCostPer1KUSD, 0.002)
	cfg.JudgeDBPath = yc.stringDefault(yc.JudgeDBPath, DefaultJudgeDBPath())
	cfg.JudgeEnabled = cfg.JudgeSampleRate > 0 && cfg.JudgeURL != "" && cfg.JudgeModel != ""

	// Routing confidence
	cfg.RoutingConfidenceDB = yc.stringDefault(yc.RoutingConfidenceDB, DefaultRoutingConfidenceDBPath())
	cfg.RoutingConfidenceFloor = yc.floatDefault(yc.RoutingConfidenceFloor, 0.4)
	cfg.RoutingConfidenceCeiling = yc.floatDefault(yc.RoutingConfidenceCeiling, 0.85)
	cfg.RoutingConfidenceMinSamples = yc.intDefault(yc.RoutingConfidenceMinSamples, 5)
	cfg.RoutingConfidenceWindow = yc.durationDefault(yc.RoutingConfidenceWindow, 168*time.Hour)

	// Health
	cfg.HealthPollInterval = yc.durationDefault(yc.HealthPollInterval, 30*time.Second)
	cfg.HealthBreakerThreshold = yc.intDefault(yc.HealthBreakerThreshold, 3)
	cfg.HealthProbeTimeout = yc.durationDefault(yc.HealthProbeTimeout, 5*time.Second)

	// Probe
	cfg.ProbePollInterval = yc.durationDefault(yc.ProbeInterval, 60*time.Second)
	cfg.ProbeTimeout = yc.durationDefault(yc.ProbeTimeout, 5*time.Second)
	cfg.ProbeBytesPerToken = yc.intDefault(yc.ProbeBytesPerToken, 256*1024)
	cfg.ProbeEnabled = cfg.ProbePollInterval > 0

	// Local concurrency
	cfg.LocalMaxConcurrent = yc.intDefault(yc.LocalMaxConcurrent, 0)
	cfg.LocalVRAMBytesPerSlot = yc.int64Default(yc.LocalVRAMBytesPerSlot, DefaultLocalVRAMBytesPerSlot)
	cfg.LocalCooldown = yc.durationDefault(yc.LocalCooldown, 10*time.Second)

	// HTTP server timeouts
	cfg.MaxBodyBytes = yc.intDefault(yc.MaxBodyBytes, DefaultMaxBodyBytes)
	cfg.ReadTimeout = yc.durationDefault(yc.ReadTimeout, DefaultServerReadTimeout)
	cfg.WriteTimeout = yc.durationDefault(yc.WriteTimeout, 0)
	cfg.IdleTimeout = yc.durationDefault(yc.IdleTimeout, DefaultServerIdleTimeout)
	cfg.MaxHeaderBytes = yc.intDefault(yc.MaxHeaderBytes, DefaultServerMaxHeaderBytes)
	cfg.ShutdownTimeout = yc.durationDefault(yc.ShutdownTimeout, DefaultShutdownTimeout)

	// Budget
	cfg.BudgetDailyLimit = yc.floatDefault(yc.BudgetDailyLimit, 0)
	cfg.BudgetAlertEnabled = yc.boolDefault(yc.BudgetAlertEnabled, false)
	cfg.BudgetAlertThreshold = clampFloat(yc.floatDefault(yc.BudgetAlertThreshold, 0.8), 0, 1)
	cfg.BudgetAlertWebhookURL = yc.BudgetAlertWebhookURL

	// Selector
	cfg.SelectorWindow = yc.durationDefault(yc.SelectorWindow, time.Hour)
	cfg.SelectorMinSamples = yc.intDefault(yc.SelectorMinSamples, 5)
	cfg.SelectorRefreshInterval = yc.durationDefault(yc.SelectorRefreshInterval, 60*time.Second)
	cfg.FrontierCostPer1K = yc.floatDefault(yc.FrontierCostPer1K, 0.005)
	cfg.ZAICostPer1K = yc.floatDefault(yc.ZAICostPer1K, 0.002)

	// Cost baseline
	cfg.CostBaselineProvider = yc.stringDefault(yc.CostBaselineProvider, "frontier")
	cfg.CostBaselineModel = yc.stringDefault(yc.CostBaselineModel, cfg.FrontierModel)
	cfg.CostBaselineRatePer1K = yc.floatDefault(yc.CostBaselineRatePer1K, cfg.FrontierCostPer1K)

	// Quality
	cfg.QualityConcurrency = yc.intDefault(yc.QualityConcurrency, 2)
	cfg.QualityQueueDepth = yc.intDefault(yc.QualityQueueDepth, 64)
	cfg.QualityTimeout = yc.durationDefault(yc.QualityTimeout, 60*time.Second)
	cfg.QualityStderrCap = yc.intDefault(yc.QualityStderrCap, 2*1024)
	cfg.QualityEnabled = cfg.QualityConcurrency > 0

	// Middleware
	cfg.PromptInjectionMode = middleware.ParseInjectionMode(yc.PromptInjectionMode)

	// Logging
	cfg.LogLevel = parseLogLevel(yc.LogLevel)
	cfg.LogFormat = parseLogFormat(yc.LogFormat)

	// Debug
	cfg.Debug = yc.boolDefault(yc.Debug, false)
	cfg.DebugBodyBytes = yc.intDefault(yc.DebugBodyBytes, DefaultDebugBodyBytes)

	// Models
	cfg.ModelsEndpointEnabled = yc.boolDefault(yc.ModelsEndpointEnabled, true)
	cfg.ModelsCacheTTL = yc.durationDefault(yc.ModelsCacheTTL, 5*time.Minute)
	...
	// Rate limit
	cfg.RateLimitRPM = yc.intDefault(yc.RateLimitRPM, 0)
	cfg.RateLimitBurst = yc.intDefault(yc.RateLimitBurst, 0)

	// RAG poll interval (needs special 0 handling)
	cfg.RAGPollInterval = yc.durationDefault(yc.RAGPollInterval, 30*time.Second)
...
func (yc YAMLConfig) stringDefaultAllowEmpty(v, def string) string {
	// Unlike stringDefault, this does NOT treat empty as "use default".
	// Used for paths that legitimately default to a file path.
	if v == "" {
		return def
	}
	return v
...
func (yc YAMLConfig) boolDefault(v, def bool) bool {
	// YAML unmarshaler treats missing field as zero value (false), so
	// we need to distinguish "explicit false" from "not set". We use a
	// pointer or check a sentinel — for simplicity we treat false as
	// "use default" unless the field was explicitly set in YAML.
	// Since we can't distinguish those cases with plain bool, we use
	// the convention that an empty YAMLConfig means all defaults.
	return v
}
...
func parseDuration(v string) (time.Duration, error) { return time.ParseDuration(v) }
